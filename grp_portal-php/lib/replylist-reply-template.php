...
$sql_reply_post = 'SELECT * FROM grape.posts WHERE posts.id = "' . $template_reply_to_id . '"';
$result_reply_post = mysqli_query($link, $sql_reply_post);
...
// Has the user given this reply a yeah?
    if(isset($_SESSION['pid'])) {	
$sql_reply_hasempathy = 'SELECT * FROM empathies WHERE empathies.id = "' . mysqli_real_escape_string($link, $template_reply_id) . '" AND empathies.pid = "' . $_SESSION['pid'] . '"';
$result_reply_hasempathy = mysqli_query($link, $sql_reply_hasempathy);
