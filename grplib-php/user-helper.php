...
global $mysql;
return array(
'news' => $mysql->query('SELECT * FROM grape.news WHERE news.to_pid = "'.$pid.'" AND news.has_read = "0" AND news.merged IS NULL ORDER BY news.news_id LIMIT 64')->num_rows,
'friend_requests' => $mysql->query('SELECT * FROM grape.friend_requests WHERE friend_requests.recipient = "'.$pid.'" AND friend_requests.has_read = "0" AND friend_requests.finished = "0"')->num_rows,
