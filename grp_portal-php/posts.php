...
if(!empty($post["topic_tag"])){
	$topic_tag = '<a href="/topics/'.urlencode($post["topic_tag"]).'" class="post-tag">'.$post["topic_tag"].'</a>';
  } else {
	$opic_tag = '';
  }
  if($post["is_ingame"] == 1){
	$played = '<div class="played" style="position:relative;top:10px;left:-8px;">&nbsp;</div>';
  } else {
	$played = '';
  }
...
if(!empty($post['screenshot'])) {
	...
	<div class="capture-container">
	 ...
	 if($post['_post_type'] == 'artwork' && str_contains($post["body"], "rvqcportal.rverse.club")) {
		print '<p class="post-content-memo"><img src="'.htmlspecialchars($post['body']).'" class="post-memo"></p></div>';
		} else if($post['_post_type'] == 'artwork' && !str_contains($post["body"], "rvqcportal.rverse.club")) { 
		  print '<p class="post-content-memo"><img src="data:image/png;base64,'.htmlspecialchars($post['body']).'" class="post-memo"></p></div>'; } else {
	 print '
...
print '<div id="post-video">
        <iframe id="post-video-player" data-video-id="'.$ytvidurl.'" frameborder="0" allowfullscreen="1" title="YouTube video player" width="900" height="504" src="https://www.youtube.com/embed/'.$ytvidurl.'?rel=0&modestbranding=0&iv_load_policy=3&enablejsapi=1&widgetid=0"></iframe>
