...
$pagetitle = loc('grp.portal.favorites_my');
} else {
$user = prepared('SELECT user_id, screen_name, pid FROM people WHERE people.pid = ?', [$is_fav_own])->fetch_assoc();
$pagetitle = sprintf(loc('grp.portal.favorites_other'), htmlspecialchars($user['screen_name'])); } }
else {
...
if(!empty($is_fav_own)) {
noContentWindow(loc('grp.portal.no_favorites_other'));
} else {
noContentWindow(loc('grp.portal.no_favorites_my')); } }
else {
