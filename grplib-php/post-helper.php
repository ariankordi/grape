<?php
$curl_array = array(CURLOPT_RETURNTRANSFER=>1,  CURLOPT_URL=> urldecode($_POST['screenshot']),  CURLOPT_CONNECTTIMEOUT=>20,  CURLOPT_USERAGENT=>'Nintendo Wii (http)',  CURLOPT_FOLLOWLOCATION=>true,  CURLOPT_HEADER=>true,  CURLOPT_CUSTOMREQUEST=>'HEAD',  CURLOPT_NOBODY=>true);
...
if(filter_var($_POST['screenshot'], FILTER_VALIDATE_URL) === FALSE) { return 'invalid'; } elseif (substr($_POST['screenshot'], 0, 5) != "https" && strlen($_POST['screenshot']) >= 3) { return 'nossl'; }
  $ch1 = curl_init(); curl_setopt_array($ch1, $curl_array);
  $content1 = curl_exec($ch1);
  $contentType1 = curl_getinfo($ch1, CURLINFO_CONTENT_TYPE);
