...
$search_frienduserlistli1 = mysqli_query($link, 'SELECT * FROM grape.friend_requests WHERE friend_requests.sender = "'.$_SESSION['pid'].'" AND friend_requests.recipient = "'.$row_user_to_view['pid'].'" AND friend_requests.finished = "0"');
...
$sql_user_to_view_profile = 'SELECT * FROM grape.profiles WHERE profiles.pid = "'.$row_user_to_view['pid'].'"';
$result_user_to_view_profile = mysqli_query($link, $sql_user_to_view_profile);
...
$result_posts_getfavoritepost = mysqli_query($link, 'SELECT * FROM grape.posts WHERE posts.id = "'.mysqli_real_escape_string($link, $row_user_to_view_profile['favorite_screenshot']).'"');
