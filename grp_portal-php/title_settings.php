...
if($search_title->num_rows == 0) { http_response_code(404); header('Content-Type: application/json; charset=utf-8');
...
if(!$set) { json500(); grpfinish($mysql); exit(); }
jsonSuccess();
} else {
...
if(!$set_create) { json500(); grpfinish($mysql); exit(); }
jsonSuccess();
}
